                      ...
                      size_t count, loff_t *f_pos)
{
  ...
  */
  err = my_spi_write_byte(spi_devs[minor].spi, value);
                     ...
                     size_t count, loff_t *f_pos)
{
  ...
  */
  my_spi_read_byte(spi_devs[0].spi, &result);

  int newResult = (u32) result * 3300 / 4096;

  if(MODULE_DEBUG)
    printk(KERN_ALERT "%s-%i read: %i\n",
           spi_devs[minor].spi->modalias, spi_devs[minor].channel, newResult);
